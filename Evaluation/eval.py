...
Adapted from https://github.com/abachaa/MEDIQA-CORR-2024/blob/main/evaluation/mediqa-corr-2024-eval-script-2-all-metrics.ipynb
...
output_file = pd.read_csv('./project_output/MEDIQA-CORR-2024-MS-ValidationSet-1-Full.pred.uw_baseline_epoch4.csv',index_col = 0)
output_file['pred_corrected_sentence'] = output_file['pred_corrected_sentence'].apply(clean_text)
output_file['Corrected Sentence'] = output_file['Corrected Sentence'].apply(clean_text)

def parse_reference_file(df):
...
def parse_run_submission_file(file):
    ...
    for index, row in file.iterrows():
        text_id = row['Text ID']
        error_flag = str(row['pred_flags'])
        predicted_flags[text_id] = error_flag
        
        if error_flag == '0':
            candidate_corrections[text_id] = "NA"
        else:
            s = row['pred_corrected_sentence']
            if isinstance(s,str):
                s = s.strip('<eos>')
            else:
                s = 'NA'
            candidate_corrections[text_id] = s
    
        predicted_flags[text_id] = str(row['pred_flags'])
        candidate_sent_id[text_id] = str(row['pred_error_sentence_id'])

    return candidate_corrections, predicted_flags, candidate_sent_id

reference_corrections, reference_flags, reference_sent_id = parse_reference_file(output_file)
candidate_corrections, candidate_flags, candidate_sent_id = parse_run_submission_file(output_file)
